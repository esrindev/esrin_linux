...
struct tok_list{
  ...
  struct tok_list *ast_block_tree;
  struct stek opn_blk_stk;
  struct stek cls_blk_stk;
  int id;
  int count;
};
...
struct f_usg_el {
	struct token *tok;
	int qyt_val; //if func rets val or not(1 if yes, 0 othervise)
};

struct f_usgs {
	struct f_usg_el  usgs[MAX_USG];
	int count;
};
...
struct qyt_el{
	struct token *tok;
	int qyt_res; //1 if yes
};

struct qyt_list{
	struct qyt_el qyts[MAX_QYT];
	int count;
};

struct func_table_el{
  struct token *tok; /* token holding name of function in declaration  */
  unsigned long hid;
  int id;
...
extern void build_exec_band_for_main(struct tok_list_el *tll, struct band_list_el band[], int *size);
extern void build_exec_band_funks(struct tok_list_el *tll, struct band_list_el band[], int *size);

extern void init_fcalls_jmp_addrs(struct band_list_el band[], int size);
extern void print_exec_band(struct band_list_el band[], int size);

extern void pop_from_stek_win(struct stek *stk, void **ptr);
extern void addto_band_new(struct tok_list_el *tll, struct band_list_el band[], int *size, int id, int hpos);
extern int matchid(int k, int tok);
extern void do_ast_job();

extern void push_to_stek(struct stek *stk, void *dptr);
extern void *pop_from_stek(struct stek *stk);
extern void init_bottom_layer_block(struct tok_list *tree);
extern void print_tok_list(struct tok_list *tlist);

extern void check_errors();
extern void check_errors_new();
extern void init_prev_links();

extern void check_errors_2();
#endif
